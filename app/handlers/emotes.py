...
async def init(bot):
    ...
    async def handler(event):
        if event.is_group:
            sender, receiver = emotes.get_targets(event)
            if sender and receiver:
                await event.reply(f"{sender} обнимает {receiver}", file=await emotes.get_gif("hug"), parse_mode="html")
    ...
    async def handler(event):
        if event.is_group:
            sender, receiver = emotes.get_targets(event)
            if sender and receiver:
                await event.reply(f"{sender} целует {receiver}", file=await emotes.get_gif("kiss"), parse_mode="html")
    ...
    async def handler(event):
        if event.is_group:
            sender, receiver = emotes.get_targets(event)
            if sender and receiver:
                await event.reply(f"{sender} кусает {receiver}", file=await emotes.get_gif("bite"), parse_mode="html")
